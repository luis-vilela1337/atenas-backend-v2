...
import { MercadoPagoNotification } from '../entities/mercado-pago-notification.entity';
  ...
  implements WebhookRepositoryInterface
{
  ...
  constructor(
    @InjectRepository(MercadoPagoNotification)
    private readonly notificationRepository: Repository<MercadoPagoNotification>,
    @InjectRepository(PaymentStatusHistory)
    private readonly paymentHistoryRepository: Repository<PaymentStatusHistory>,
  ...
  async updatePaymentStatus(
    ...
    const historyEntity = this.paymentHistoryRepository.create({
      ...
      transactionAmount: status.transactionAmount,
      dateApproved: status.dateApproved,
      dateCreated: status.dateCreated,
      lastModified: status.lastModified,
    });

    await this.paymentHistoryRepository.save(historyEntity);
  }
