...
async function seed() {
  ...
  if (!allowedEnvs.includes(env)) {
    console.error(
      `❌ Seed bloqueado! Ambiente "${env}" não permitido. Apenas: ${allowedEnvs.join(
        ', ',
      )}`,
    );
    process.exit(1);
  ...
  console.log(`🌱 Iniciando seed do banco de dados (env: ${env})...\n`);
  ...
  try {
    ...
    if (!institution) {
      institution = await institutionRepo.save({
        ...
        observations:
          'Instituição criada automaticamente para testes e desenvolvimento',
      });
      console.log(
        `✅ Instituição criada: ${institution.name} (${institution.contractNumber})`,
      );
    } else {
    ...
    if (!client) {
      ...
      console.log(
        `✅ Cliente criado: ${client.email} / senha: senha123 (R$ 500 crédito)`,
      );
    } else {
    ...
    const products = [
      {
        ...
        photos: [
          'https://via.placeholder.com/400x300/4CAF50/FFFFFF?text=Album',
        ],
        video: [],
      },
      {
        ...
        photos: [
          'https://via.placeholder.com/400x300/2196F3/FFFFFF?text=Fotos',
        ],
        video: [],
      },
      {
        ...
        photos: [
          'https://via.placeholder.com/400x300/FF9800/FFFFFF?text=Digital',
        ],
        video: [],
      },
    ];
    ...
    const institutionProductRepo =
      dataSource.getRepository('InstitutionProduct');
    ...
    const productConfigs = [
      // ALBUM
      {
        ...
        details: {
          minPhoto: 30,
          maxPhoto: 50,
          valorEncadernacao: 150.0, // R$ 150 encadernação
          valorFoto: 5.0, // R$ 5 por foto
        },
      },
      // GENERIC
      {
          ...
          events: [
            {
              ...
              name: 'Formatura',
              minPhotos: 1,
              valorPhoto: 10.0, // R$ 10 por foto
              valorPack: 200.0, // R$ 200 pacote completo do evento
            },
            {
              id: createdEvents[1].id,
              name: 'Baile de Gala',
              minPhotos: 1,
              valorPhoto: 8.0, // R$ 8 por foto
              valorPack: 150.0, // R$ 150 pacote completo do evento
            },
          ],
        },
      },
      // DIGITAL_FILES
      {
        ...
        details: {
          ...
          valorPackTotal: 300.0, // R$ 300 pack completo de tudo
          events: [
            {
              id: createdEvents[0].id,
              name: 'Formatura',
              valorPhoto: 3.0, // R$ 3 por foto digital
              valorPack: 100.0, // R$ 100 todas digitais do evento
            },
            {
              id: createdEvents[1].id,
              name: 'Baile de Gala',
